...
let upperLimit = Number(prompt("Enter first number: "));
let lowerLimit = Number(prompt("Enter second number: "));
if(upperLimit > lowerLimit){
    for (let i = upperLimit; i >= lowerLimit; i -= 10) {
        console.log(i);
    }
} else {
    alert("Error: the first number must be greater than the second number");
}
...
for (number of numbers) {
    console.log(number > 10 && number < 60 ? number : "");
}

//____________________________________________________________________________________________________//

/* Exercise 4: Write a program using a loop that will ask the user for the name of the movie (first prompt)
and its rating from www.imdb.com (second prompt). The program will allow you to enter as many movies as you
want into the movies array. Each element of the array will be an object, consisting of two fields: title and imdb.
The input is completed if the user presses Cancel in the prompt dialog. Then the program should first print out to
the console all movies that have a rating of less than 7, then those whose rating is greater than or equal to 7.
...
/* Exercise 6: Modify the calculator program that you made in Module 4 Section 2 in such a way
that it will work in the loop until the user inputs Q in any of the prompt boxes. */
